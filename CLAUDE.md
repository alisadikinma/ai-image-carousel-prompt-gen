...
- **Scene-Override Costume (rare, v3)**: Costume priority: user override > scene-override > signature outfit. Only a deck whose narrative is a literal specific environment uses scene-override; otherwise the signature outfit is the default. Topic-category costume switching is RETIRED.
- **Signature Outfit (v3.0, pipeline + interactive)**: The creator wears ONE **signature smart-casual outfit** (dark tee/henley + unstructured blazer, neutral tone) on EVERY slide where the creator face appears (cover/human_fingerprint/cta), every topic. **Topic is conveyed by the floating topic elements, never by costume.** Resolution: (1) **scene-override** (rare literal-setting deck → `source = scene_override`); (2) **signature outfit** (default — almost always → `source = signature`). No topic-keyword match, no LLM costume inference, no per-profession archetype switching. Replaces the v2.20–v2.25 topic-immersive costume machinery
- **Spotlight Portrait Template Gate (v3.0)**: Every cover MUST be the Spotlight Portrait template — solid blue `#0F59B6` base + top-bar swipe pill + calm creator portrait in the signature outfit + **≥3 floating topic UI elements** (real tool cards/logos/screenshots) + headline with 2-4 gold accent words + CTA pill. The **floating elements are the scroll-stop**, not an absurd scene. A cover with <3 floating elements or not on the blue base AUTO-FAILS (SKILL.md Rule #17 + Cover Self-Check Gate). Headline stays serious/professional (Headline Independence Rule). Replaces the v2.20–v2.25 absurdist 8-tier dramatic-hook ranking
- **`creator_outfit` schema field (optional)**: Top-level envelope field on `CompleteEnvelopeSchema` — `{ category, prompt_phrase, source: 'signature' | 'scene_override' | 'topic_match' | 'fallback', reasoning }`. v3 default is `source: 'signature'` (`'scene_override'` for rare literal-setting decks); `'topic_match'`/`'fallback'` retained DEPRECATED for pre-v3 backward-compat. Captures which costume the pipeline picked + why for admin UI audit. Optional for backward compat. Plus envelope `notes[]` log lines: `costume_resolved: <category> via <source>` + `visual_hook_resolved: Spotlight Portrait ("<floating elements>")` + `cover_gate: pass`
...
When `/carousel-gen` runs non-interactively (auto-detected via `--blog-source`, `--pipeline`, `--non-interactive` flags or no-TTY), it cannot Read references on demand. The compiled bundle at `references/compiled/refs-carousel-gen-pipeline.md` (~280KB after v3.0, was ~281KB at v2.25, ~169KB pre-v2.20) is injected via `--append-system-prompt-file` and contains:

| Reference | Purpose in pipeline |
|---|---|
| `global-config.md` | Creator identity, language, colors (blue base `#0F59B6` + gold accent), bilingual chrome, format defaults |
| `creator-bible.md` | Signature outfit default, settings, cool-neutral+gold-rim lighting, public-figure rules |
| **`hook-visual-library.md`** | **Spotlight Portrait Template + Floating Topic Elements spec (v3)** + §10 Signature Outfit + Scene → Costume Override Table (scene-override only) + §11 prop interactions. *(v3: §1 hook-category selection archived/non-default; §10 Topic-Keyword Resolution Table + Conceptual Archetypes removed.)* |
| `hook-formula-bank.md` | 52 hook formula templates |
| `cinematography-lut.md` | Lighting + camera + lens lookup |
| `prompt-formulas.md` | Prompt body rendering rules + Spotlight Portrait Layout (top-bar pill, ≥3 floating, CTA pill, blue base) |
| `carousel-best-practices.md` | Platform specs, 5-act narrative spine |
| `non-interactive-defaults.md` | Pipeline resolution rules (v3) — costume §2 (signature-outfit chain), visual hook §5 (**≥3 floating topic elements** hard rule), §3 cover setting = Spotlight Portrait template, §5c Topic → Floating Elements hint table |
...
| Cover not on blue base / missing floating elements | **v3 Spotlight Portrait template gate (Rule #17).** Every cover MUST be on the solid blue `#0F59B6` base with a top-bar swipe pill, creator portrait in the signature outfit, **≥3 floating topic UI elements**, headline with 2-4 gold accent words, CTA pill. A cover with <3 floating elements or not on the blue base AUTO-FAILS. Verify: bundle contains "Spotlight Portrait" + "floating"; `notes[]` logs `visual_hook_resolved: Spotlight Portrait (...)` + `cover_gate: pass` |
| Floating elements too few / generic | Hard rule = **≥3 floating topic elements** (real tool cards/logos/screenshots the content names — prefer specific tools over generic). See `hook-visual-library.md` "Floating Topic Elements" + `non-interactive-defaults.md` §5c Topic → Floating Elements hint table |
| Creator wearing topic-costume (doctor/astronaut/blazer-per-topic) | **v3 retired topic-costume switching (Rule #18).** The creator wears ONE **signature smart-casual outfit** (dark tee/henley + unstructured blazer) on every slide, every topic — topic shows in the floating elements. Verify `creator_outfit.source = signature`; `notes[]` logs `costume_resolved: Signature smart-casual via signature`. Only a rare literal scene-override deck uses `source = scene_override` |
| Cover renders warm-amber / absurd scene (not blue portrait) | **v3 reversal.** The absurdist MAX-chaos scene + warm Kodak palette is RETIRED. Verify `global-config.md` has `background_base #0F59B6` + cool-neutral grade (no "Warm golden amber"), and the deployed bundle is post-v3 (~280KB, contains "Spotlight Portrait", NOT "Memory Architect"/"resolved absurd"). Re-run `npm run compile-refs` and redeploy the VPS symlink if stale |
| `creator_outfit` field missing from envelope | Optional field on `CompleteEnvelopeSchema`. Pre-feature drafts won't have it — graceful skip. New v3 runs SHOULD populate `{ category, prompt_phrase, source: 'signature' or 'scene_override', reasoning }`. Backend adapter (`CarouselGenOutputAdapter` in Portfolio_v2) graceful-skips; `'topic_match'`/`'fallback'` still parse for pre-v3 backward compat |
| CTA flat / not driving Save | **v3 Rule #21 CTA Action variant.** CTA slide = deepened-navy `cta_background` + gold glow, signature outfit, "join me" open-palm gesture, floating = mini value-recap, engagement ask prioritizing **Save + Comment + Share**. Comment-to-DM is deferred (headline-swappable later). Check `notes[]` + the CTA Social Media Block |
...
**Version:** 3.0.0
**Last Updated:** June 12, 2026 — **Spotlight Portrait blue rebrand + floating-UI hook (v3.0.0).** Major visual-identity reversal: the v2.20→v2.25 MAX-chaos absurdist hook machinery + per-topic costume switching is **RETIRED** and replaced by the **Spotlight Portrait** template — a calm credible creator portrait + **≥3 floating topic UI elements** (real tool cards/logos/screenshots) on a **solid signature blue `#0F59B6`** base, with one **signature smart-casual outfit** (dark tee/henley + unstructured blazer) used on every slide and every topic. Topic is conveyed by the floating elements, never by costume. **Why:** the absurdist direction fought Ali's "Strategic Industrial AI Architect — builder, not influencer/hype" positioning; a clean blue (trust/enterprise) portrait fits better and DigitalRajas-style floating-UI cards are the new scroll-stop. **Changes (all plugin-side, no Portfolio_v2 backend change):** **(config)** `global-config.md` §3/§4 — `background_base #0F59B6`, `cta_background` (deepened navy + gold glow), `cta_engagement_actions` (Save + Comment + Share; comment-to-DM deferred/swappable), `cta_pill_text`; `color_grade` → cool-neutral key + blue ambient + gold rim; `color_temp` → ~5000-5600K; `film_stock` → clean digital; bilingual chrome `swipe_cta_text` "GESER 👆 / swipe". **(pipeline)** `non-interactive-defaults.md` — §2 costume → signature-outfit chain (scene-override rare → signature default; topic-keyword/LLM-inference removed); §3 cover row → "= Spotlight Portrait template"; §5 → floating-element resolution with **hard rule ≥3 floating topic elements** (absurdist 8-tier ranking removed); §5c → Topic → Floating Elements hint table (replaces the Conceptual-Topic→Metaphor-Scene table). **(visual lib)** `hook-visual-library.md` — new top "Spotlight Portrait Template" section + Floating Topic Elements spec; §10 Topic-Keyword Resolution Table + 8 Conceptual Archetypes (Memory Architect … Concept Avatar) REMOVED; Signature Outfit card added; profession cards demoted to non-default scene-override reference; §1 hook-category selection ARCHIVED/non-default. **(skill)** `SKILL.md` — Rule #17 → Spotlight Portrait template gate (cover AUTO-FAILS if <3 floating elements / not on blue base); Rule #18 → signature outfit (no topic-costume / Concept Avatar); Cover Self-Check Gate → template-compliance (still logs `cover_gate`); new Rule #21 CTA Action variant; Step 7c → Spotlight Portrait variations. **(layout)** `prompt-formulas.md` — Spotlight Portrait Layout + CTA Action-variant layout. **(wardrobe)** `creator-bible.md` §6 → Signature Outfit; §4 palette/film → blue base + cool-neutral; topic-costume coupling dropped. **(schema)** `schema.ts` — `CreatorOutfitSchema.source` += `'signature'` (v3 default); `'topic_match'`/`'fallback'` kept DEPRECATED for pre-v3 backward-compat; field stays optional; `image_prompt` cap unchanged at 1800. **(tests)** `refs-content.test.ts` rewritten to pin the Spotlight Portrait contract + assert absurdist anchors are gone; `schema.test.ts` + `'signature'` source tests; `golden/draft28-equivalent.json` regenerated to Spotlight Portrait prose (≤1800 chars/slide, `creator_outfit.source='signature'`, beats + bilingual copy preserved). **(bundle)** regenerated **281KB → 280KB** (contains "Spotlight Portrait" + "floating"; NOT "Memory Architect" / "resolved absurd"). **Acceptance:** `npm test` green (refs-content + schema + golden + compile-refs); `npx tsc --noEmit` clean. **Out of scope:** animation (handled by `ai-video-promo-engine` — separate brainstorm). **Operator action required:** on VPS plugin cache → `git pull` (branch `feat/spotlight-portrait-blue-rebrand`) → `npm install && npm run compile-refs` → verify symlink `/home/claudesn/refs-carousel-gen-pipeline.md` resolves to the new ~280KB bundle (contains "Spotlight Portrait", not "Memory Architect"). No Laravel `.env` change, no backend restart. Design doc `docs/plans/2026-06-12-spotlight-portrait-blue-rebrand.md`.

---

**Version:** 2.25.0
**Earlier (June 11, 2026)** — **Scene-first max-chaos hook + topic-immersive conceptual costume (v2.25.0).** *(SUPERSEDED by v3.0.0 above — the absurdist hook + topic-costume machinery described below is RETIRED.)* The v2.24 "Second Brain" LinkedIn deck still rendered a conservative explainer-diagram cover + navy blazer despite the v2.20 dramatic-hook machinery. **Two root causes:** (1) `non-interactive-defaults.md` §3 mood-to-setting table hard-coded the `cover` setting as "warm-lit modern studio + Edison-bulb bokeh" — a concrete default that overrode the more abstract Rule #17 absurdist mandate; (2) abstract/conceptual topics (second brain, PKM, privacy, AI, workflow, mindset, deep-work, build-in-public) matched no §10 profession keyword, so costume fell through to a tame business-casual default. **Fix (4 commits, all plugin-side, no Portfolio_v2 backend change):** mechanism shift to **scene-first** — resolve the dramatic absurd scene FIRST, then BOTH setting and costume derive from it. **C1** `non-interactive-defaults.md`: §3 cover row → "= the resolved absurd §5 scene (no generic-studio default)"; §5 → MAX-absurdist-chaos-but-topic-anchored intensity directive (≥1 recognizable topic element so it reads intentional, not random "Visual Shock"; headline stays professional per Headline Independence); new **§5c Conceptual-Topic→Metaphor-Scene table** (7 clusters + Concept Avatar). **C2** `hook-visual-library.md` §10: new "Conceptual / Metaphor Archetypes" block — 8 theatrical character costumes (**Memory Architect, Data Guardian, AI Whisperer, Systems Engineer, Mind Hacker, Signal Cutter, Builder/Maker, Concept Avatar** fallback), each with full Top/Bottom/Accessories/Texture/Color/Prompt-phrase rows + Default-absurd-scene; 7 new conceptual keyword rows in the Topic Keyword→Category Resolution Table; `second brain`/`deep work` de-duped from the Productivity row so first-match-wins routes them to Memory Architect/Signal Cutter. **C3** `SKILL.md`: Rule #17 strengthened (scene-first, generic-studio AUTO-FAIL, topic-anchored); Rule #18 strengthened (abstract topics → §10 Conceptual Archetype / Concept Avatar, NEVER blazer); new **Cover Self-Check Gate** callout (pre-emit scene + costume validation, logs `cover_gate` to `notes[]`, both modes); Step 7c interactive parity. **C4** version 2.24.0→2.25.0 (`package.json`, `.claude-plugin/plugin.json`, lockfile); bundle regenerated **252KB→281KB**. **TDD:** new `skills/carousel-gen/refs-content.test.ts` (vitest, 18 content-guard assertions wired to the real ref files, RED→GREEN per phase). **Acceptance:** `npm test` 48/48 green (refs-content 18, schema 25, golden 3, compile-refs 2 — the 3 golden failures noted in the v2.20 entry below were already resolved by commit `9bd55bb`); `npx tsc --noEmit` clean; bundle contains "Memory Architect" + "Concept Avatar" + "resolved absurd". Implemented via `/gaspol-brainstorm` → `/gaspol-plan` → `/gaspol-execute` (sequential, per-phase checkpoints); design doc `docs/plans/2026-06-11-scene-first-max-chaos-hook.md` (gitignored). **Operator action required (Phase E, manual):** on VPS plugin cache → `git pull` (branch `feat/scene-first-max-chaos-hook`) → `npm install && npm run compile-refs` → verify symlink `/home/claudesn/refs-carousel-gen-pipeline.md` resolves to the new ~281KB bundle (contains "Memory Architect") → re-roll the Second Brain deck to validate the cover is now absurdist + archetype-costumed. No Laravel `.env` change, no backend restart. Note: `package.json`/`plugin.json` had drifted to 2.24.0 (commits 2.21–2.24) while this footer still claimed 2.20.0 — now re-aligned at 2.25.0.
