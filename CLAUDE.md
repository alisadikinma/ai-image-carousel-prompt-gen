...
When `/carousel-gen` runs non-interactively (auto-detected via `--blog-source`, `--pipeline`, `--non-interactive` flags or no-TTY), it cannot Read references on demand. The compiled bundle at `references/compiled/refs-carousel-gen-pipeline.md` (~252KB after v2.20, was ~169KB) is injected via `--append-system-prompt-file` and contains:
...
| **`hook-visual-library.md`** *(added v2.20)* | §1 absurd hook categories + §10 costume library + §11 prop interactions |
...
| `non-interactive-defaults.md` | Pipeline resolution rules — costume §2 (4-step priority chain), visual hook §5 (8-tier dramatic ranking) |
