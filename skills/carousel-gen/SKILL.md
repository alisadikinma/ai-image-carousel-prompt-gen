...
18. **Signature outfit (every creator-face slide).** Every slide where the creator face appears (cover / human_fingerprint / cta) uses the ONE **signature smart-casual outfit** (dark tee/henley + unstructured blazer, neutral tone — `hook-visual-library.md` §10 Signature Outfit card), consistent across every slide and every topic. Topic is conveyed by the **floating topic elements**, NEVER by costume. There is no topic-keyword costume switching in v3. The only exception is a rare **scene-override** deck (literal setting — `creator_outfit.source = scene_override`); otherwise `creator_outfit.source = signature`
...
> **Costume resolution per slide (MANDATORY)** — For every slide where the creator face appears (cover, human_fingerprint, cta layouts), resolve the `[Wardrobe]` slot to the **signature smart-casual outfit** (`hook-visual-library.md` §10 Signature Outfit card — dark tee/henley + unstructured blazer, neutral tone), consistent across every slide and every topic. There is NO topic-keyword costume switching in v3 — topic is conveyed by the floating elements.
>
