...
3. **Interactive Slide Design (ambiguity)** → resolve costume via the v3 signature-outfit chain (scene-override rare → signature outfit default) per `references/non-interactive-defaults.md` §2. Log resolved costume + source to `notes[]` and populate envelope `creator_outfit` field (`source = signature` by default).
4. **Hook Clarification (Step 7b)** → resolve the cover to the **Spotlight Portrait template** (calm creator portrait + ≥3 floating topic UI elements on the blue base) per `hook-visual-library.md` "Spotlight Portrait Template"; log to `notes[]`.
5. **Visual Hook Idea (Step 7c)** → choose the **≥3 floating topic elements** (real tools/logos/screenshots of the topic) that surround the creator; cover slide MUST carry ≥3 floating elements (see Visual Hook + Costume Resolution section below); log to `notes[]`.
...
17. **Spotlight Portrait template gate (cover).** Every cover MUST be the Spotlight Portrait template (`hook-visual-library.md` "Spotlight Portrait Template"): solid blue `#0F59B6` base + top-bar swipe pill + calm credible creator portrait in the signature outfit + **≥3 floating topic UI elements** (real tool cards/logos/screenshots of the topic) + bottom-gradient headline with 2-4 gold accent words + bottom CTA pill. A cover **missing the floating elements (<3), not on the blue base, or not on the template AUTO-FAILS** — re-author it before emit. The scroll-stop is the floating topic elements, NOT an absurd scene. The hook headline stays serious/professional (Headline Independence Rule) while the floating UI grabs the eye
18. **Signature outfit (every creator-face slide) — with the locale exception.** Every slide where the creator face appears (cover / human_fingerprint / cta) uses the ONE **signature smart-casual outfit** (dark tee/henley + unstructured blazer, neutral tone — `hook-visual-library.md` §10 Signature Outfit card) by default, consistent across slides. Topic is conveyed by the **floating topic elements**, never by per-keyword costume. Resolve `[Wardrobe]` via §2's chain: (a) **locale/culture override** — if the topic is fundamentally about a specific country/culture (the place *is* the story), the creator wears that locale's **modern, credible traditional attire** (e.g. kurta for India), `creator_outfit.source = locale_override`, applied to ALL creator-face slides in the deck for consistency; (b) **scene-override** rare literal-setting deck, `source = scene_override`; (c) otherwise `source = signature`. A passing country mention does NOT trigger the locale override
...
20. **BODY + PEAK = educational explainer cards (content-dense, face-free)** — In the default `cinematic` deck, Act 3 (BODY) and Act 4 (PEAK) slides are NOT cinematic hero photos carrying a single big word/number. They are **editorial knowledge cards that teach** — this is where the audience must actually learn something. Every BODY and PEAK slide MUST render IN-IMAGE: **(a)** one **teaching headline** — the single concrete idea, specific not vague ("RAG cuts hallucination by grounding answers in your own docs", NEVER a bare "938%"); **(b)** **2-3 supporting context sub-points**, each a short labeled line carrying a real mechanism / number / step / example that explains WHY and HOW (this is the *essence + context* the viewer takes away); **(c)** one **takeaway line** ("so for you: …" / "artinya: …") so the card stands alone as a complete lesson. A lone statistic AUTO-FAILS — a number must always be paired with *what it means* and *why it matters*. Visual treatment = a cohesive branded **icon-led infographic / knowledge-card layout**: clean editorial grid, **one distinct icon for EVERY sub-point** (clear flat line/duotone glyph in brand `accent_color`) + a **header icon or micro-diagram** for the teaching headline + a **mini visual** (bar/ring/arrow/gauge) for any stat — favor an icon-grid / icon-row / numbered-icon-list so the brain digests the card at a glance (minimum 3 icons per card; a text-only card AUTO-FAILS), generous whitespace, strong type hierarchy (headline » sub-points » takeaway) — still 4K hyperrealistic-grade composition and lighting on the card surface, **NOT hand-drawn** (the doodle look is the separate `--style=sketchnote` preset). **No creator face — and no human figure at all** on BODY/PEAK cards (per Rules #3/#4 — content slides without human figures = no face), so they carry NO `creator` token and need no `[Wardrobe]` resolution; even on public-figure topics the body/peak card renders their logo/product/data, never a portrait. Mirror the full teaching text into the slide's `copy` (or `copy_id` + `copy_en`) so captions/metadata reflect the depth. **Prompt budget**: the explainer card still fits the 1800-char `image_prompt` cap — it has headroom precisely because it DROPS the cinematic-portrait prose (wardrobe, floating elements, 8-element WOW). Spend the budget on the teaching text + a clean layout description, not on photographic mood. **The creator face appears on EXACTLY THREE acts — HOOK (Act 1), FORESHADOW (Act 2), CTA (Act 5)** — cinematic creator-face hero shots per Rules #3/#18/#19. The face → infographic → infographic → face rhythm IS the visual signature of the deck. **PEAK carries NO face** — it is a face-free explainer card like BODY (the former `human_fingerprint` PEAK portrait exception is removed; PEAK proof = data/visual, not a creator portrait).
21. **CTA = Spotlight Portrait Action variant.** The last (CTA) slide uses the Spotlight Portrait template with these overrides: background = the deepened **navy variant of `#0F59B6`** + extra gold glow (`cta_background` — signals "action/close" while the signature blue stays intact, NOT dark-inverted); creator half-body in the signature outfit with a warm confident "join me" expression and an **open-palm / pointing gesture** toward the engagement actions; floating elements = a **mini value-recap** of the carousel (the tools/points covered) as glassy cards so there's a reason to Save (≥3 retained); optional hand-drawn dashed arrow. The engagement ask is **exactly ONE command** — pick a single action (default **SIMPAN/Save**; or BAGIKAN/Share, SUKA/Like, KOMENTAR/Comment) and render it as the **largest, boldest, most mencolok** element on the slide. ONE slide = ONE ask; **NEVER stack two commands** (no "Save + Share", no "SIMPAN & BAGIKAN"). **Comment-to-DM is deferred** — when the platform is ready, swap the single ask to a "Komentar '[WORD]' → DM" reward (the proven Engagement Reward type, `hook-science.md`, 12-18%) without changing the visual template. Include the `global-config.md` CTA Social Media Block.
22. **People Spotlight — profile slides need REAL faces (overrides #20 for named people).** A body slide that PROFILES specific real people — headline `SIAPA <Name>?` / "Who is X?", or copy centring on named founders / CEOs / authors — is the documented exception to the face-free body-card rule (#20). For such a slide: emit the **people_spotlight contract** (`needs_real_faces:true` + `people[]` of real name(s)+role + `face_layout:'photo_band_top'`, see `schema.ts` + `creator-bible.md` → "People Spotlight"), **reserve a clear empty photo band** in the `image_prompt` for real face cut-outs (framed pinned-polaroid treatment), and **do NOT doodle, draw, or invent the named person** — the consumer composites their real photo into the band. **Never render the person's real NAME as on-image text** — the name lives only in `people[]`. Applies in BOTH `cinematic` and `sketchnote` styles (only the framing differs). A regular concept/teaching body card (no named people) stays face-free per #20.
...
> **Visual Hook authoring (MANDATORY for cover slide)** — author the cover as the **Spotlight Portrait template** (`hook-visual-library.md` "Spotlight Portrait Template"): a calm credible creator portrait on the solid blue `#0F59B6` base, surrounded by **≥3 (3–6) floating topic UI elements** — glassy translucent cards / screenshots / logos of the *real* tools/objects/concepts the content discusses, with soft glow + depth blur. Optional small AI-robot mascot for AI topics.
>
> **Hard rule**: every cover slide image_prompt MUST carry **≥3 floating topic elements** on the blue base + the top-bar swipe pill + headline with 2-4 gold accent words. The floating elements are the scroll-stop; an absurd scene is NOT used in v3. Fewer than 3 floating elements, or a non-blue base, AUTO-FAILS. The hook headline stays serious/professional (Headline Independence Rule) while the floating UI grabs the eye.
>
> Append a `visual_hook_resolved: Spotlight Portrait ("<floating elements chosen>")` line to envelope `notes[]`.

> **Costume resolution per slide (MANDATORY)** — For every slide where the creator face appears (cover, human_fingerprint, cta layouts), resolve the `[Wardrobe]` slot via the §2 chain: **signature smart-casual outfit** by default (`hook-visual-library.md` §10 Signature Outfit card — dark tee/henley + unstructured blazer, neutral tone). The ONE topic-driven exception is the **`locale_override`** — when the topic is fundamentally about a specific country/culture, dress the creator in that locale's modern traditional attire (e.g. kurta for India), applied consistently to every creator-face slide in the deck. There is NO per-keyword costume switching in v3 — topic is otherwise conveyed by the floating elements.
>
> The only exception is a rare **scene-override** deck whose narrative is literally set in a specific environment (`source = scene_override`); otherwise the signature outfit is `source = signature`.
>
> After resolving, populate the envelope's top-level `creator_outfit` field with `{ category: 'Signature smart-casual', prompt_phrase, source: 'signature' | 'scene_override', reasoning }`. Also append a `costume_resolved: <category> via <source>` line to `notes[]`.

> **Cover Self-Check Gate (MANDATORY — run before emitting the JSON envelope)** — validate the cover/HOOK slide against BOTH template-compliance checks; if EITHER fails, re-author the cover before emit:
>
> 1. **Template check** — is the cover on the solid blue `#0F59B6` base with a top-bar swipe pill, a creator portrait, **≥3 floating topic UI elements**, a headline with 2-4 gold accent words, and a CTA pill? Fewer than 3 floating elements, a non-blue base, or a missing top-bar pill FAILS.
> 2. **Costume check** — is the creator in the **signature smart-casual outfit** (not a topic-switched profession costume), unless a genuine scene-override applies? A topic-switched costume FAILS.
>
> On pass, append `cover_gate: pass` to envelope `notes[]`. On a re-authored fix, append `cover_gate: re-authored (<reason>)`. This gate applies in BOTH pipeline and interactive modes. Confirm the cover carries ≥3 floating elements before logging pass.
...
|---------|-----------|-------------------|
| **Profession/costume** | Scene is a literal specific environment that might demand a setting outfit | "Slide N is set in [context]. v3 default is the signature smart-casual outfit (topic shows in the floating elements). This scene might call for a scene-override costume — want the signature outfit or a scene-override?" (see Visual Hook + Costume Resolution section above) |
...
7c. **VISUAL HOOK IDEA** — present 3 **Spotlight Portrait variations** (2-3 sentence pitches): each is the creator in the signature outfit on the blue base, differing in the **creator's simple action** and the **set of ≥3 floating topic UI elements** (which real tools/logos/screenshots surround them). NOT absurd scenes. Same template + costume rules as pipeline (Rules #17/#18 + Cover Self-Check Gate). User picks, modifies, or provides own idea
